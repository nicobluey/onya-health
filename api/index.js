...
} from '../backend/lib/patient-details.js';
import {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && routePath === 'patient/profile') {
      ...
      const patientDetailsValidation = validateCertificatePatientDetails({
        fullName,
        dob,
        email: requestedEmail || currentEmail,
        phone,
        address,
      });
      if (!patientDetailsValidation.valid) {
        sendJson(res, 400, {
          error: patientDetailsValidation.errors[0],
          code: 'PATIENT_DETAILS_REQUIRED',
          details: patientDetailsValidation.errors,
        });
        return;
      }
    ...
    if (req.method === 'GET' && routePath === 'patient/clinical-profile') {
      const patient = await requirePatient(req, res);
      if (!patient) return;
    ...
    if (req.method === 'POST' && routePath === 'patient/clinical-profile') {
      const patient = await requirePatient(req, res);
      if (!patient) return;
      const body = await parseJsonBody(req);
    ...
    if (req.method === 'POST' && routePath === 'patient/clinical-profile/test-results') {
      const patient = await requirePatient(req, res);
      if (!patient) return;
      const body = await parseJsonBody(req);
    ...
    if (
      ...
      segments.length === 5 &&
      segments[0] === 'patient' &&
      segments[1] === 'clinical-profile' &&
      segments[2] === 'test-results' &&
      segments[4] === 'file'
    ) {
      const patient = await requirePatient(req, res);
      if (!patient) return;
      ...
      sendJson(res, 200, {
        ...download,
        expiresInSeconds: isSupabaseStorageEnabled() ? 300 : null,
      });
      return;
    }
    ...
    if (
      ...
      segments[0] === 'doctor' &&
      segments[1] === 'patients' &&
      segments[3] === 'test-results' &&
      segments[5] === 'file'
    ) {
      const doctor = await requireDoctor(req, res);
      if (!doctor) return;
      const lookup = await resolveDoctorPatientLookup(decodeURIComponent(segments[2]));
      if (!lookup) {
        sendJson(res, 404, { error: 'Patient not found' });
        return;
      }
      const recordId = decodeURIComponent(segments[4]);
    ...
    if (req.method === 'POST' && segments.length === 4 && segments[0] === 'doctor' && segments[1] === 'certificates' && segments[3] === 'decision') {
      ...
      const updated = await updateCertificate(certId, (item) => {
        ...
        return {
          ...
          status: decision,
          certificateDraft: nextDraft,
          rawSubmission: mergeCertificateDraftIntoSubmission(item.rawSubmission, nextDraft, workflowPatch),
          decision: {
            by: reviewerName,
            byEmail: normalizeEmail(doctor.email),
            providerType: String(doctorProfile?.providerType || '').trim(),
