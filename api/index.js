...
function sendJson(res, statusCode, payload) {
  setCors(res);
  const requestHeaders = res?.req?.headers;
  ...
  res.setHeader('ETag', `W/"no-cache-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}"`);
  ...
  try {
    ...
    if (req.method === 'GET' && routePath === 'patient/meal-plan/catalog') {
      ...
      const includeDataImages = ['1', 'true', 'yes'].includes(
        String(url.searchParams.get('includeDataImages') || '')
          .trim()
          .toLowerCase()
      );
    ...
    if (req.method === 'GET' && routePath === 'patient/meal-plan/latest') {
      const patient = await requirePatient(req, res);
      if (!patient) return;
      const includeDataImages = !['0', 'false', 'no'].includes(
        String(url.searchParams.get('includeDataImages') || '')
          .trim()
          .toLowerCase()
      );
