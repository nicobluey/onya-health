...
export const SelectableCard = ({ selected, onClick, children, className }: SelectableCardProps) => (
    <div
        onClick={onClick}
        className={cn(
            "cursor-pointer border-2 p-4 transition-all duration-200 flex items-center justify-between",
            selected
                ? "border-primary bg-[#edf2ff]"
                : "border-border bg-white hover:border-primary hover:bg-[#edf2ff]",
            className
        )}
    >
        <div className="font-extrabold text-text-primary">{children}</div>
    ...
    ({ className, label, error, ...props }, ref) => {
        return (
            <div className="w-full space-y-1.5">
                {label && <label className="block text-sm font-extrabold text-text-secondary">{label}</label>}
                <input
                    ref={ref}
                    className={cn(
                        "flex h-12 w-full border border-border bg-white px-3 py-2 text-sm ring-offset-white file:border-0 file:bg-transparent file:text-sm file:font-bold placeholder:text-bark-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:border-transparent disabled:cursor-not-allowed disabled:opacity-50 transition-all",
...
export const Checkbox = ({ checked, onChange, label }: CheckboxProps) => (
    <label className="flex items-start space-x-3 cursor-pointer group">
        <div className="relative flex items-center">
            ...
            />
            <div className={cn(
                "h-5 w-5 border transition-all flex items-center justify-center",
    ...
    <AnimatePresence>
        {isOpen && (
            <>
                ...
                >
                    <div className="onya-panel pointer-events-auto w-full max-h-[calc(100dvh-4rem)] overflow-y-auto md:max-h-[calc(100dvh-6rem)] md:max-w-[640px]">
                        {children}
                    </div>
                </motion.div>
            </>
